...
void GraphModelGraphicsItem::createMolecule()
{
    ...
    // qDebug() << "star" <<m_model->getStars().size();
    while(sit.hasNext())
    {
        const QList<EdgeModel> &bonds = sit.next();
        QPointF Scenecenter,starbond;
        // qDebug() << bonds.size();
        ...
        for(int i=0; i<bonds.size(); ++i)
        {
            ...
            Scenecenter += bond->mapToScene(bond->boundingRect().center());
        }
        Scenecenter /= bonds.size();
        for(int i=0; i<bonds.size(); ++i)
        {   
            starpath.moveTo(Scenecenter.x(), Scenecenter.y());

            BondItem* bond = m_edge_bond_mapping[bonds[i]];
            starbond = (bond->mapToScene(bond->boundingRect().center()) - Scenecenter)*2;
            starpath.lineTo(Scenecenter.x()+starbond.x(),Scenecenter.y()+starbond.y());
        }
    }
